...
Route::namespace('\App\PaymentGateways')->group(function () {
    ...
    // Stripe
    Route::prefix('intasend')->group(function () {

        Route::get('/', 'intasend\IntasendController@index')
            ->name('intasend');

        Route::post('process', 'intasend\IntasendController@capturePayment')
            ->name('intasend_process');
    });
