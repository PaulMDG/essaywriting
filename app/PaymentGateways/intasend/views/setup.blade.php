<form role="form" class="form-horizontal" enctype="multipart/form-data" action="{{ route('patch_settings_intasend') }}" method="post" autocomplete="off" >
   {{ csrf_field()  }}
   {{ method_field('PATCH') }}
   ...
   <div class="form-group">
      <label>Public APIKey <span class="required">*</span></label>
      <input type="text" class="form-control {{ showErrorClass($errors,'publicAPIKey') }}" name="publicAPIKey" value="{{ old('publicAPIKey', optional($rec->keys)->publicAPIKey) }}">
      <div class="invalid-feedback">{{ showError($errors,'publicAPIKey') }}</div>
   </div>
   <div class="form-group">
